...
<div class="modal fade" id="deletePropertyFileModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                <button type="button" class="btn btn-danger" id="confirmDeletePropertyFileBtn">Delete</button>
            </div>
        </div>
    </div>
</div>
...
async function deletePropertyFile(fileId, fileType) {
    __pendingDeletePropertyFile = { fileId: fileId, fileType: fileType };
    const mEl = document.getElementById('deletePropertyFileModal');
    if (mEl && window.bootstrap && window.bootstrap.Modal) {
        window.bootstrap.Modal.getOrCreateInstance(mEl).show();
    }
}

document.getElementById('confirmDeletePropertyFileBtn')?.addEventListener('click', async function(){
    const pending = __pendingDeletePropertyFile;
    __pendingDeletePropertyFile = null;
    const mEl = document.getElementById('deletePropertyFileModal');
    ...
    try {
        const response = await fetch(`${BASE_URL}/files/delete/${pending.fileId}`, {
            method: 'POST',
            headers: {
                'X-Requested-With': 'XMLHttpRequest',
